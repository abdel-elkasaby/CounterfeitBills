...
def write_results_to_csv(image_dir, csv_path):
    image_paths = list(Path(image_dir).glob('*.jpg'))
    results = []

    for image_path in image_paths:
        has_black_stripe, _ = detect_black_stripe(image_path)
        results.append([image_path.name, int(has_black_stripe)])

    # Write results to CSV
    with open(csv_path, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['image_name', 'has_black_stripe'])  # Write headers
...
class ImageNavigator:
    def __init__(self, image_dir, csv_path):
        ...
        self.prev_button = tk.Button(self.root, text="<", command=self.prev_image)
        self.prev_button.pack(side="left")

        self.next_button = tk.Button(self.root, text=">", command=self.next_image)
        self.next_button.pack(side="right")

        self.correct_button = tk.Button(self.root, text="This has a stripe", command=self.mark_as_stripe)
    ...
    def update_image(self):
        ...
        has_black_stripe, processed_image = detect_black_stripe(image_path)
        ...
        if image_path.name in self.corrections:
            corrected_result = self.corrections[image_path.name]
            if corrected_result:
        ...
        self.current_detection = has_black_stripe
    ...
    def mark_as_stripe(self):
        image_path = self.image_paths[self.current_index]
        self.corrections[image_path.name] = True
        self.update_image()

    def mark_as_no_stripe(self):
        image_path = self.image_paths[self.current_index]
        self.corrections[image_path.name] = False
        self.update_image()
    ...
    def calculate_summary(self):
        ...
        for image_path in self.image_paths:
            ...
            original_detection, _ = detect_black_stripe(image_path)
            corrected_detection = self.corrections.get(image_name, original_detection)
...
# Write results to CSV before starting the GUI
write_results_to_csv(image_dir, csv_path)
